"use client";
// Wholesale cart page island. Renders the localStorage wholesale cart with
// qty steppers + remove, MOQ enforcement warnings, and a sticky checkout bar.
...
interface WholesaleCartIslandProps {
  tenantSlug: string;
}

export function WholesaleCartIsland({ tenantSlug }: WholesaleCartIslandProps) {
  const d = useDict();
    ...
    return (
      <div className="mx-auto flex max-w-storefront flex-col items-center gap-4 px-4 py-16 text-center">
        <p className="bn-body text-lg font-semibold text-ink">
          🛒 আপনার পাইকারি কার্ট খালি
        </p>
        <p className="bn-body text-sm text-ink-muted">
          পাইকারি পণ্য ব্রাউজ করে কার্টে যোগ করুন
        </p>
        <a href="/wholesale">
  ...
  return (
    <div className="mx-auto max-w-storefront px-4 pb-28 pt-4">
      <h1 className="bn-heading mb-4 text-xl font-bold text-ink">
        🏭 পাইকারি কার্ট
      </h1>
        ...
        {cart.lines.map((line) => (
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              <p className="bn-body line-clamp-1 text-sm font-medium text-ink">
                {line.title}
              </p>
              {line.variantTitle && (
      ...
      <div className="fixed inset-x-0 bottom-0 z-sticky border-t border-border bg-surface shadow-lg">
        <div className="mx-auto flex max-w-storefront items-center gap-3 px-4 py-2.5">
          ...
          <a href="/wholesale/checkout" className="flex-1">
